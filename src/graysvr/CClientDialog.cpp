...
size_t CDialogDef::GumpAddText(LPCTSTR pszText)
{
	...
	m_sText[m_iTextsIndex] = pszText;
	m_iTextsIndex++;

	if (m_iTextsIndex > m_iTextsCount)
		m_iTextsCount = m_iTextsIndex;

	return (m_iTextsIndex - 1);
}
...
bool CDialogDef::r_Verb(CScript& s, CTextConsole* pSrc, CScriptTriggerArgs* pArgs, CDialogDefSetup* pDlgSetup)	// some command on this object as a target
{
	...
	case GUMPCTL_DTEXT:
	{
		if (m_iControls >= (COUNTOF(m_sControls) - 1))
			return false;
		if (m_iTextsCount >= (COUNTOF(m_sText) - 1))
			return false;
	...
	case GUMPCTL_SETTEXT:
	{
		...
		if (m_iTextsCount < textIndex + 1)
			m_iTextsCount = textIndex + 1;
		m_iTextsIndex = textIndex;
		return true;
	}
...
bool CDialogDef::GumpSetup(int iPage, CClient* pClient, CObjBase* pObjSrc, LPCTSTR Arguments)
{
	...
	if (g_Cfg.ResourceLock(s, RESOURCE_ID(RES_DIALOG, GetResourceID().GetResIndex(), RES_DIALOG_TEXT)))
	{
		...
		while (s.ReadKey())
		{
			if (m_iTextsCount >= (COUNTOF(m_sText) - 1))
				break;
			m_pObj->ParseText(s.GetKeyBuffer(), pClient->GetChar(), 0, &Args);
			m_sText[m_iTextsIndex] = s.GetKey();
			m_iTextsIndex++;
			if (m_iTextsCount < m_iTextsIndex)
				m_iTextsCount = m_iTextsIndex;
		}
	}
...
bool CClient::Dialog_Setup(CLIMODE_TYPE mode, RESOURCE_ID_BASE rid, int iPage, CObjBase* pObj, LPCTSTR Arguments)
{
	...
	addGumpDialog(mode, pDlg->m_sControls, pDlg->m_iControls, pDlg->m_sText, pDlg->m_iTextsCount, pDlg->m_x, pDlg->m_y, pObj, context);
	return true;
}
